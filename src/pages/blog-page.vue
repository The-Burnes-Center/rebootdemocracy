...
export default {
  ...
  computed: {
    ...
    filteredTagDataWithoutNews() {
      return this.filteredTagData.filter(
        (tag) => tag !== "News that caught our eye"
      );
    },
  },
  created() {
  ...
  methods: {
    ...
    includesString(array, string) {
      ...
      return array.some((s) =>
        s.toLowerCase().includes(lowerCasePartialSentence)
      );
    },
    resetSearch() {
      this.blogDataSearch = [];
        ...
        .then((item) => {
          ...
          item.data.map((tag) => {
            tag?.Tags?.map((subTags) => {
              if (
                subTags != null &&
                !this.inclucesString(this.filteredTagData, subTags)
              ) {
          ...
          this.filteredTagData.unshift(
            this.filteredTagData.splice(
              this.filteredTagData.indexOf("News that caught our eye"),
              1
            )[0]
          );
...
<template>
  ...
  <div class="blog-page-hero">
    ...
    <a href="/signup" class="btn btn-small btn-primary"
  ...
  >
    <div class="blog-featured-row">
        ...
        <a :href="'/blog/' + latestBlogPost.slug">
          <img
            ...
            :src="
              this.directus._url +
              'assets/' +
              blogData.slice().reverse()[0].image.id +
              '?width=800'
            "
          />
          ...
          <p>
            Published on {{ formatDateOnly(new Date(latestBlogPost.date)) }}
          </p>
          <div class="author-list">
            <p class="author-name">
              {{ latestBlogPost.authors.length > 0 ? "By" : "" }}
            </p>
            <div v-for="(author, i) in latestBlogPost.authors">
              <div class="author-item">
                ...
                <div class="author-details">
                  <p class="author-name">
                    {{ author.team_id.First_Name }}
                    {{ author.team_id.Last_Name }}
                  </p>
                  <p
                    class="author-name"
                    v-if="
                      latestBlogPost.authors.length > 1 &&
                      i < blogData.slice().reverse()[0].authors.length - 1
                    "
                  >
                    and
                  </p>
      ...
      <div
        class="other-blog-posts"
        v-if="!searchResultsFlag || searchTerm == ''"
      >
        <div
        ...
        >
          <a :href="'/blog/' + blog_item.slug">
            <img
              ...
              class="blog-list-img"
              :src="this.directus._url + 'assets/' + blog_item.image.id"
            />
