              ...
              { status: { _eq: 'published' } },
              {
                _or: [
                  ...
                  { content: { _contains: searchTermClean } },
                  {
                    authors: {
                      _some: {
                        team_id: { First_Name: { _contains: searchTermClean } },
                      },
                    },
                  },
                  {
                    authors: {
                      _some: {
                        team_id: { Last_Name: { _contains: searchTermClean } },
                      },
                    },
                  },
                  {
                    authors: {
                      _some: {
                        team_id: { Title: { _contains: searchTermClean } },
                      },
                    },
                  },
                ],
...
<template>
    ...
    <div class="blog-featured-row">
      <div class="first-blog-post" v-if="latestBlogPost">
        <a :href="'/blog/' + latestBlogPost.slug">
          <div v-lazy-load>
            <!-- Updated: Use filename_disk and Netlify images proxy with width=800 -->
            <img v-if="latestBlogPost.image" class="blog-list-img" :data-src="`/.netlify/images?url=${encodeURIComponent('https://thegovlab-files.nyc3.cdn.digitaloceanspaces.com/thegovlab-directus9/uploads/' + blogData.slice().reverse()[0].image.filename_disk)}&w=800`">
          </div>
          ...
          <div class="author-list">
            ...
            <div v-for="(author, i) in latestBlogPost.authors" :key="i">
      ...
      <div class="other-blog-posts" v-if="!searchResultsFlag || searchTerm == ''">
        <div class="other-post-row" v-for="(blog_item, index) in blogData.slice().reverse()" v-show="index > 0 && index < 4"> 
          <a :href="'/blog/' + blog_item.slug">
            <div v-lazy-load>
              <!-- Updated: Use filename_disk for other posts -->
              <img v-if="blog_item.image" class="blog-list-img" :data-src="`/.netlify/images?url=${encodeURIComponent('https://thegovlab-files.nyc3.cdn.digitaloceanspaces.com/thegovlab-directus9/uploads/' + blog_item.image.filename_disk)}&w=800`">
            </div>
  ...
  <div v-if="!searchResultsFlag && searchTermDisplay == ''" class="allposts-section">
    <div class="allposts-post-row" v-for="(blog_item, index) in blogDataSearch.slice().reverse()" v-show="index >= 4 && index < 16">
      <a :href="'/blog/' + blog_item.slug">
        <div v-lazy-load>
          <!-- Updated: Use filename_disk for latest posts -->
          <img v-if="blog_item.image" class="blog-list-img" :data-src="`/.netlify/images?url=${encodeURIComponent('https://thegovlab-files.nyc3.cdn.digitaloceanspaces.com/thegovlab-directus9/uploads/' + blog_item.image.filename_disk)}&w=800`">
        </div>
          ...
          <div class="tag-posts-row-container">
            <div v-for="(blog_item, index) in blogDataSearch.slice().reverse()" :key="index" class="tag-posts-row">
              <div v-if="includesString(blog_item?.Tags, tag_item)">
                <a :href="'/blog/' + blog_item.slug">
                  <div v-lazy-load>
                    <!-- Updated: Use filename_disk for tag posts -->
                    <img v-if="blog_item.image" class="blog-list-img" :data-src="`/.netlify/images?url=${encodeURIComponent('https://thegovlab-files.nyc3.cdn.digitaloceanspaces.com/thegovlab-directus9/uploads/' + blog_item.image.filename_disk)}&w=800`">
                  </div>
                  <div class="allposts-post-details">
                    <h3>{{ blog_item.title }}</h3>
  ...
  <div v-if="searchResultsFlag && searchTermDisplay != ''" class="allposts-section">
    <div class="allposts-post-row" v-for="(blog_item, index) in blogDataSearch">
      <a :href="'/blog/' + blog_item.slug">
        <div v-lazy-load>
          <!-- Updated: Use filename_disk for search results -->
          <img v-if="blog_item.image" class="blog-list-img" :data-src="`/.netlify/images?url=${encodeURIComponent('https://thegovlab-files.nyc3.cdn.digitaloceanspaces.com/thegovlab-directus9/uploads/' + blog_item.image.filename_disk)}&w=800`">
        </div>
        <div class="allposts-post-details">
          <p class="post-date">Published on {{ formatDateOnly(new Date(blog_item.date)) }} </p>
