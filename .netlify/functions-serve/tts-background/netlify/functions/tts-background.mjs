...
async function runProcess(bodyres) {
  try {
    let getFullName2 = function(author) {
      return `${author.team_id.First_Name} ${author.team_id.Last_Name}`;
    }, joinAuthorNames2 = function(authors2) {
      if (authors2.length === 1) {
        return getFullName2(authors2[0]);
      } else {
        return authors2.map(getFullName2).join(", ").replace(/, (?=[^,]+$)/, " and ");
      }
    };
    var getFullName = getFullName2, joinAuthorNames = joinAuthorNames2;
 ...
 ${joinAuthorNames2(authors).length > 0 ? "\nby" : ""} ${joinAuthorNames2(authors)} 
