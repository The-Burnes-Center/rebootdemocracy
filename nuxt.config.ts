...
export default defineNuxtConfig({
  ...
  routeRules: {
    ...
    '/blog': { prerender: true },
    // Blog posts - ISR (Incremental Static Regeneration) with cache tags
    // Hybrid approach:
    // 1. Initial build: All blog posts are "warmed up" via warm-up script (generated on first request)
    // 2. Updates: When content changes, cache is purged → page regenerates on next request via ISR
    ...
    // - Page is generated by serverless function on first request (or after cache purge)
    // - Generated page is cached at CDN edge with cache tags
    // - When cache is purged via webhook, cache is cleared
    // - On next request, page regenerates via serverless function (not a full build)
