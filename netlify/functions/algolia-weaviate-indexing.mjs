...
async function upsertBlog(id) {
  ...
  const verification = await wv.graphql
    .get()
    .withClassName('RebootBlogPostChunk')
    .withFields('slug directusId title part')
    .withWhere({
      path: ['directusId'],
      operator: 'Equal',
  ...
  console.log('Verification of indexed content:');
  console.log('Directus slug:', rec.slug);
  console.log('Weaviate chunks:', verification.data.Get.RebootBlogPostChunk);
}
